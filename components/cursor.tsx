...
export const CursorProvider = ({ children }: CursorProviderProps) => {
	...
	return (
		<CursorContext.Provider value={{ onCursor }}>
			<div
				className={`hidden xl:block fixed top-0 left-0 z-50 pointer-events-none w-8 h-8 rounded-full border border-secondary transition-all duration-300 ease-out ${
					expandCursor ? 'opacity-20' : 'opcaity-100'
				}`}
				style={{ transform: `translate(${x - 16}px, ${y - 16}px)` }}
			></div>
			<div
				className={`hidden xl:block  fixed top-0 left-0 z-50 pointer-events-none  rounded-full bg-secondary  transition-all duration-400 ease-out ${
