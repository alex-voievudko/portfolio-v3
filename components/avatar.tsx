...
export const Avatar = () => {
	...
	return (
		<div className='relative w-24 h-24 rounded-full border-4 border-primary flex-shrink-0  overflow-hidden'>
			{transitions((style, index) => (
				<animated.div style={style} className='absolute p-2 inset-0 w-full h-full flex items-center justify-center'>
					<Image src={images[index]} alt='Avatar' priority className='object-cover w-full rounded-full h-full' />
				</animated.div>
			))}
		</div>
	);
